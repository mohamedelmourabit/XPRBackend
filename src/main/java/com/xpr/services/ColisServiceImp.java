...
public class ColisServiceImp implements ColisService {
	...
	public Colis saveColis(Colis colis) {
		...
		if (colis.getLigneColis().isEmpty()) {

			LigneColis lc = new LigneColis();

			Produit produit = new Produit();
			produit.setClient(colis.getClient());
			produit.setSku(colis.getProduit());
			produit.setPrixVente(colis.getPrix());
			produit.getColis().add(colis);
			produit = produitRepository.save(produit);

			Variante variante = new Variante();
			variante.setProduit(produit);
			variante.setSku(colis.getProduit());
			variante.setPrix(colis.getPrix());
			variante.setQte(1);

			variante = varianteRepository.save(variante);

			lc.setVariante(variante);
			lc.setColis(colis);
			lc.setQte(1);
			colis.getLigneColis().add(lc);

		}

		Historique h = Historique.getHistorique("Ajout d'un nouveau colis: " + colis.getNumCommande(),
				colis.getStatut(), "cniTest");
		h.setColis(colis);
	...
	public Colis updateStatutColis(String numCommande, String statut) throws ColisException {
		...
		if (colis.getStatut().equals(Constants.NOUVEAU_COLIS)
			...
			colis.setDateModification(new Date());
			colis = colisRepository.save(colis);
			Historique h = Historique.getHistorique("Modification statut du colis: " + colis.getNumCommande(),
					colis.getStatut(), "cniTest");
			h.setColis(colis);
			historiqueRepository.save(h);
			return colis;
	...
	public List<Colis> affectationColisToLivreur(String emailLivreur, List<Colis> colis) throws LivreurException {
		...
		for (Colis coli : colis) {
			...
			Historique h = Historique.getHistorique("Affectation du colis au livreur: " + coli.getNumCommande(),
					coli.getStatut(), "cniTest");
	...
	public Colis affectationColisToLivreur(String cniLivreur, Colis colis) throws LivreurException {
		...
		if (livreur == null) {
			...
			Historique h = Historique.getHistorique("Affectation du colis au livreur: " + colis.getNumCommande(),
					colis.getStatut(), "cniTest");
			colis = colisRepository.save(colis);
	...
	public Colis affectationColisToRamasseur(String cniRamasseur, Colis colis) throws LivreurException {
		...
		if (livreur == null) {
			...
			Historique h = Historique.getHistorique("Affectation du colis au rammasseur: " + colis.getNumCommande(),
					colis.getStatut(), "cniTest");
			colis = colisRepository.save(colis);
			colis.getHistoriques().add(h);
		}
	...
	public Colis desaffectationColisToLivreur(String cniLivreur, Colis colis) throws LivreurException {
		...
		Historique h = Historique.getHistorique("Désaffectation du livreur: " + cniLivreur, colis.getStatut(),
				"cniTest");
		colis = colisRepository.save(colis);
